...
def _make_request(token: str | None = None, remote_addr: str = "1.2.3.4") -> MagicMock:
    request = MagicMock()
...
def test_same_user_different_tokens_same_key(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.middleware.rate_limit._lookup_user_id",
        lambda token: 42,
    )
...
def test_lookup_user_id_caches(monkeypatch) -> None:
    """Successful lookups should be cached."""
    _token_user_cache.clear()
    monkeypatch.setattr(
        "app.middleware.rate_limit.sqlite3.connect",
        _fake_connect_factory({"tok-1": 99}),
    )
    assert _lookup_user_id("tok-1") == 99
