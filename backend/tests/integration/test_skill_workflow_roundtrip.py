...
def build_workflow_graph(skill_name: str) -> dict[str, Any]:
    return {
        "nodes": [
...
@pytest.mark.asyncio
async def test_skill_create_and_execute_in_workflow_roundtrip(
    client: AsyncClient,
) -> None:
    login_resp = await client.post(
        "/api/v1/auth/register",
        json={"email": f"rt-{uuid4().hex[:8]}@example.com", "password": "password123"},
    )
    ...
    try:
        ...
        done_events = [
            payload for event_name, payload in events if event_name == "done"
        ]
        assert done_events
